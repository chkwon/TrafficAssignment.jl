function ta_mcp(ta_data; method=:bfw, max_iter_no=2000, step=:exact, log=:off, tol=1e-3)
    ...
    @variable(m, mu[i=1:length(nodes), w=1:length(od_pairs)] >=0)
    @NLexpression(m, travel_time[a=1:length(links)],
          free_flow_time[a] * ( 1 + B[a] *
            ( sum( x[a,ww] for ww=1:length(od_pairs)) / capacity[a] )^power[a] )
    )
    @mapping(m, flow_balance[i=1:length(nodes), w=1:length(od_pairs)],
            sum( delta[i,a] * x[a,w] for a=1:length(links) ) - q[i,w]
    )
    @mapping(m, wardrop[a=1:length(links), w=1:length(od_pairs)],
            travel_time[a] + mu[links[a][2],w] - mu[links[a][1],w]
    )
    @complementarity(m, wardrop, x)
    @complementarity(m, flow_balance, mu)

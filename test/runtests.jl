...
link_flow, link_travel_time, objective = ta_frank_wolfe(ta_data, method=:bfw, step=:exact, log=:on, tol=1e-6, max_iter_no=100)

@show objective

link_flow2, link_travel_time2, objective2 = ta_nlp(ta_data, method=:bfw, step=:exact, log=:on, tol=1e-6, max_iter_no=100)
